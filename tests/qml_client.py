...
from PySide6.QtCore import Q_ARG, QCoreApplication, QEvent, QMetaObject, Qt, QUrl
...
class Client:
    def __init__(self, url, key="", timeout_ms=1000):
        ...
        self.component = QQmlComponent(self.engine, QUrl.fromLocalFile(str(ROOT / "Controller.qml")))
        if self.component.isError():
            raise RuntimeError("\n".join(error.toString() for error in self.component.errors()))
        self.object = self.component.create()
        if self.object is None:
            raise RuntimeError("\n".join(error.toString() for error in self.component.errors()))
        for name, value in {"checkUpdates": False, "baseUrl": url, "apiKey": key,
                            "requestTimeoutMs": timeout_ms, "pollSeconds": 3600}.items():
            self.set(name, value)
    ...
    def call(self, name, *args):
        if not QMetaObject.invokeMethod(self.object, name, Qt.DirectConnection,
    ...
    def wait(self, predicate, timeout=15):
        ...
        while time.monotonic() < until:
            ...
            if predicate():
                return
            time.sleep(0.01)
        raise AssertionError(f"QML client timed out: {self.get('error')} {self.get('actionError')}")
