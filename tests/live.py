...
def main():
    ...
    try:
        ...
        assert (
            client.get("health")["version"].removeprefix("v") == expected
        ), "Runner is not using the requested Lemonade release"
        assert not client.get("actionError"), client.get("actionError")
        assert not client.get("modelsError"), client.get("modelsError")
        if model:
            assert any(
                item["id"] == model for item in client.get("models")
            ), "Provision the test model through Lemonade before running"
            ...
            assert any(
                item["model_name"] == model for item in loaded
            ), "Model not reflected in the plugin"
            device = os.environ.get("LEMONADE_TEST_DEVICE", "")
            if device:
                assert any(
                    item["model_name"] == model
                    and device in item.get("device", "").split()
                    for item in loaded
                ), "Expected accelerator was not reported"
        print(
            f"QML client compatible with Lemonade {expected}; model={model or '(API only)'}"
        )
    finally:
        if loaded_here:
            ...
            assert not client.get("actionError"), client.get("actionError")
            assert not any(
                item["model_name"] == model for item in client.get("loadedModels")
            ), "Test model did not unload"
        client.close()
