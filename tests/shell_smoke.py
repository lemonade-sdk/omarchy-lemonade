...
def wait_online():
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if state["online"] and state["version"].removeprefix("v") == os.environ["LEMONADE_RELEASE"].removeprefix("v"):
                return state
        except (subprocess.SubprocessError, ValueError, KeyError):
            pass
        time.sleep(1)
    raise AssertionError(f"Shell plugin did not connect to the expected release: {last}")


def main():
    config = Path(os.environ.get("XDG_CONFIG_HOME", str(Path.home() / ".config")))
    if config.resolve() != (Path.home() / ".config").resolve():
        raise RuntimeError("Omarchy's plugin removal command requires the standard ~/.config location")
    plugins = (config / "omarchy" / "plugins").resolve()
    destination = plugins / PLUGIN_ID
    if destination.exists() or destination.is_symlink():
        raise RuntimeError("Use a dedicated test session without an existing Lemonade plugin")
    ...
    try:
        ...
        run("omarchy", "bar", "set", PLUGIN_ID, "baseUrl", os.environ.get("LEMONADE_URL", "http://localhost:13305"))
        ...
        run("omarchy", "plugin", "enable", PLUGIN_ID)
        wait_online()
        print("Omarchy plugin installed, connected, accepted panel IPC, and re-enabled successfully")
    finally:
