...
def main():
    ...
    headers = {"Accept": "application/vnd.github+json", "User-Agent": "omarchy-lemonade-ci"}
    if os.environ.get("GH_TOKEN"):
        headers["Authorization"] = "Bearer " + os.environ["GH_TOKEN"]
    request = urllib.request.Request(f"https://api.github.com/repos/lemonade-sdk/lemonade/releases/tags/{tag}", headers=headers)
    with urllib.request.urlopen(request, timeout=30) as response:
        release = json.load(response)
    assert not release["draft"] and not release["prerelease"]
    system = "windows-x64.zip" if platform.system() == "Windows" else "ubuntu-x64.tar.gz"
    ...
    with tempfile.TemporaryDirectory(prefix="omarchy-lemonade-") as directory:
        ...
        with archive.open("rb") as source:
            assert hashlib.file_digest(source, "sha256").hexdigest() == digest[7:], "Release asset checksum mismatch"
        ...
        binary = next(extracted.rglob("lemond.exe" if platform.system() == "Windows" else "lemond"))
        cache = folder / "cache"
        config = folder / "config"
        cache.mkdir()
        config.mkdir()
        with (results / "lemond.log").open("w", encoding="utf-8") as log:
            server = subprocess.Popen([str(binary), str(cache), str(config), "--host", "127.0.0.1", "--port", "13389"], stdout=log, stderr=subprocess.STDOUT)
            try:
                for _ in range(60):
                    if server.poll() is not None:
                        raise RuntimeError("Release server exited; see lemond.log")
                    try:
                        request = urllib.request.Request(os.environ["LEMONADE_URL"] + "/v1/health", headers={"Authorization": "Bearer " + os.environ["LEMONADE_API_KEY"]})
